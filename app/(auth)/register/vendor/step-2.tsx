...
import { useVerifyCodeVendorMutation } from "@/store/auth/api";
...
export default function VendorRegStep2() {
  const router = useRouter();

  const { phone } = useLocalSearchParams();
  const { t } = useLocale();

  const [verifyCode, { isLoading }] = useVerifyCodeVendorMutation();
      ...
      >
        <View style={styles.codeContainer}>
          ...
          <Text style={styles.instructionText}>
            {t("vendorPhoneVerification.verificationCode.instruction")}
          </Text>

          {errors.code && (
            <Text style={styles.errorText}>
              {errors.code?.message ||
                t("vendorPhoneVerification.verificationCode.codeRequired")}
            </Text>
          )}
          ...
          <ButtonPrimary
            ...
            isLoading={isLoading}
          >
            {t("vendorPhoneVerification.verificationCode.verifyButton")}
          </ButtonPrimary>
        </View>
